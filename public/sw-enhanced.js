...
// Font patterns
const FONT_PATTERNS = [/\.(woff2?|ttf|eot|otf)$/i];
...
class CacheStrategy {
  ...
  static async cacheFirst(request, cacheName = STATIC_CACHE_NAME) {
    ...
    if (cachedResponse) {
      ...
      const isStale =
        cacheDate && Date.now() - parseInt(cacheDate) > CACHE_DURATIONS.STATIC;
...
class PDFCacheManager {
  ...
  static async getCachedPDF(request) {
    ...
    if (cachedResponse) {
      const cacheDate = cachedResponse.headers.get("sw-cache-date");
      const isExpired =
        cacheDate && Date.now() - parseInt(cacheDate) > CACHE_DURATIONS.PDF;
...
async function handleOfflinePDFDownload() {
  try {
    ...
    const pendingDownloads = JSON.parse(
      localStorage.getItem("pendingPDFDownloads") || "[]",
    );
    ...
    for (const download of pendingDownloads) {
      try {
        ...
        const updatedDownloads = pendingDownloads.filter(
