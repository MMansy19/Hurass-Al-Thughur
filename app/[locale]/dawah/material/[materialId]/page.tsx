...
async function getMaterialInfo(
  materialId: string,
  locale: string,
): Promise<DawahMaterial | null> {
  const materials: Record<string, DawahMaterial> = {
    "islam-basics": {
      id: "islam-basics",
      title:
        locale === "ar"
          ? "أساسيات الإسلام - دليل المبتدئين"
          : "Islam Basics - Beginner's Guide",
      description:
        locale === "ar"
      ...
      tags:
        locale === "ar"
          ? ["أساسيات", "مبتدئين", "عقيدة"]
          : ["basics", "beginners", "faith"],
    ...
    "quran-intro": {
      ...
      content: {
        ...
        sections: [
          ...
          {
            title:
              locale === "ar" ? "معجزة القرآن" : "The Miracle of the Quran",
            content:
              locale === "ar"
...
export async function generateMetadata({
  ...
  if (!material) {
    return {
      ...
      description:
        messages.pdfViewer?.requestedMaterialNotFound ||
        "The requested dawah material was not found.",
    };
  }
...
export default async function DawahMaterialPage({
  ...
  const skipLinks = [
    ...
    },
    {
      ...
      label: isArabic ? "انتقل إلى محتوى المادة" : "Skip to material content",
    },
    {
      href: "#download-section",
      label: isArabic ? "انتقل إلى قسم التحميل" : "Skip to download section",
    },
    {
      href: "#related-materials",
      label: isArabic
        ? "انتقل إلى المواد ذات الصلة"
        : "Skip to related materials",
    },
  ];
