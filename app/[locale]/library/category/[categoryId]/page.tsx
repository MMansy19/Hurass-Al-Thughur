...
export default async function LibraryCategoryPage({
  ...
  const categoryName =
    library.categoryNames[categoryId as keyof typeof library.categoryNames];
  ...
  const categoryItems: LibraryItem[] = [
    {
      ...
      description: isArabic
        ? "مطوية تعريفية بأساسيات التوحيد في الإسلام"
        : "Introductory brochure on the basics of Tawheed in Islam",
    ...
    },
    {
      ...
      description: isArabic
        ? "مجموعة صور تعليمية عن أركان الإسلام الخمسة"
        : "Educational images about the five pillars of Islam",
      category: categoryName,
      type: "image",
    ...
    },
    {
      ...
      description: isArabic
        ? "كتاب مفصل في شرح العقيدة الإسلامية"
        : "Detailed book explaining Islamic creed",
      category: categoryName,
      type: "pdf",
      fileSize: "3.5 MB",
      pageCount: 120,
    ...
    <main className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50">
      <SkipLinks
        links={[
          ...
          {
            href: "#search",
            label: isArabic ? "الانتقال إلى البحث" : "Skip to search",
          },
        ]}
      />
      ...
      <div id="main-content" className="container mx-auto px-4 py-8">
        ...
        >
          <ol
            className={`flex items-center space-x-2 text-sm ${isArabic ? "flex-row-reverse space-x-reverse" : ""}`}
          >
            <li>
              <Link
                href={`/${locale}`}
        ...
        <Motion delay={0.3}>
          <Suspense fallback={<MagazineGridSkeleton />}>
            ...
            <AccessibleGrid
              columns={3}
              navigationLabel={
                isArabic ? `شبكة ${categoryName}` : `${categoryName} grid`
              }
              locale={locale}
            >
                ...
                >
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                      ...
                      {item.pageCount && (
                        <span>
                          {item.pageCount} {isArabic ? "صفحة" : "pages"}
                        </span>
                      )}
                    </div>

                    <div
                      className={`flex gap-3 ${isArabic ? "flex-row-reverse" : ""}`}
                    >
                      <Link
