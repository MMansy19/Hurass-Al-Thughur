...
export const IntegratedMagazineGrid = memo<IntegratedMagazineGridProps>(
  ({
    ...
    if (isLoading) {
      return (
        <MagazineGridSkeleton count={6} columns={columns as 1 | 2 | 3 | 4} />
      );
    }
      ...
      return (
        <Motion preset="fadeIn" className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900  mb-2">
            {messages?.pdfViewer?.noMagazinesAvailable ||
              (isArabic ? "لا توجد مجلات متاحة" : "No magazines available")}
          </h3>
          <p className="text-gray-500 ">
            {messages?.pdfViewer?.checkBackLater ||
              (isArabic
                ? "تحقق مرة أخرى لاحقاً للحصول على محتوى جديد"
    ...
    const MagazineCard = enableAccessibilityFeatures
      ? AccessibleMagazineCard
      : AnimatedMagazineCard;
    const GridContainer = enableAccessibilityFeatures
      ? AccessibleGrid
      : AnimatedGrid;
...
export const IntegratedSearch = memo<IntegratedSearchProps>(
  ({
    ...
    const defaultPlaceholder = useMemo(
      () =>
        placeholder ||
        (isArabic ? "البحث في المجلات..." : "Search magazines..."),
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-gray-500">
                        {messages?.pdfViewer?.loadingDocument ||
                          (isArabic
                            ? "يرجى الانتظار أثناء إعداد المستند..."
...
export const IntegratedHeaderNav = memo<IntegratedHeaderNavProps>(
  ({ navItems, enableAnimations = true, className = "" }) => {
    if (enableAnimations) {
      return <AnimatedNavigation items={navItems} className={className} />;
    }

    return (
      <nav
        className={`flex space-x-6 rtl:space-x-reverse ${className}`}
        role="navigation"
      >
        {navItems.map((item) => (
          <a
