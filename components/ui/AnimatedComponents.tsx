...
export const AnimatedMagazineCard = memo<AnimatedMagazineCardProps>(
  ({ issue, readNowText, onReadClick, delay = 0 }) => {
            ...
            <Motion preset="slideInUp" delay={delay + 0.7}>
              ...
              >
                <div className="flex items-center justify-center">
                  <svg
                    className="w-5 h-5 mx-2 rtl:mx-2 rtl:mr-0"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
...
export const AnimatedGrid = memo<AnimatedGridProps>(
  ({ children, columns = 3, gap = 24, staggerDelay = 0.1, className = "" }) => {
    const { trackClass } = useCSSOptimization();
    ...
    return (
      <Motion preset="fadeIn" className={gridClasses} style={{ gap }}>
        <Stagger staggerDelay={staggerDelay}>{children}</Stagger>
      </Motion>
    );
  },
);
  ...
  ({ type = "cards", count = 6, className = "" }) => {
    ...
    const renderSkeleton = () => {
      if (type === "cards") {
        return Array.from({ length: count }, (_, i) => (
          <Motion key={i} preset="fadeIn" delay={i * 0.1} className="space-y-4">
      ...
      if (type === "list") {
        return Array.from({ length: count }, (_, i) => (
          ...
          >
            <div
              className={`${skeletonClasses} w-16 h-16 rounded-full flex-shrink-0`}
            />
    ...
    return (
      <div className={className}>
        <Stagger staggerDelay={0.05}>{renderSkeleton()}</Stagger>
      </div>
    );
  },
);
