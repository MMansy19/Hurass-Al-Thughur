...
export const FocusManager = memo<FocusManagerProps>(
  ({ children, trapFocus = false, restoreFocus = true, autoFocus = false }) => {
  ...
  ({ issue, onDownload, locale, delay = 0, readNowText, downloadText }) => {
    ...
    const ariaLabel = useMemo(() => {
      const parts = [
        ...
        issue.date,
        issue.fileSize &&
          `${isArabic ? "حجم الملف" : "File size"}: ${issue.fileSize}`,
        issue.pageCount &&
          `${isArabic ? "عدد الصفحات" : "Pages"}: ${issue.pageCount}`,
      ].filter(Boolean);
...
export const AccessibleGrid = memo<AccessibleGridProps>(
  ({ children, columns = 3, gap = 24, navigationLabel, locale = "en" }) => {
    ...
    return (
      <div
        ...
        role="grid"
        aria-label={
          navigationLabel || (isArabic ? "شبكة المحتوى" : "Content grid")
        }
        tabIndex={-1}
      >
