  ...
  return (
    <nav className="hidden xl:flex items-center justify-between flex-grow mx-6">
      <ul className="flex items-center space-x-1 rtl:space-x-reverse lg:space-x-2">
        {navLinks.map((link, index) => (
          <li key={index}>
            ...
            >
              <span className={`${isArabic ? 'ml-2' : 'mr-2'} ${link.isActive ? 'text-white' : 'text-emerald-600 group-hover:text-emerald-700'}`}>{link.icon}</span>
      ...
      <div className="flex gap-1">
        <Link href={`${oppositeLocale === 'en' ? '/ar/dashboard' : '/en/dashboard'}`} className="flex items-center justify-center transition-colors w-10 h-10 bg-emerald-600 text-white rounded-md hover:bg-emerald-700">
...
export const LanguageSwitch = ({ oppositeLocale, pathname, isArabic, switchText, isMobile = false }: LanguageSwitchProps) => {
  const GlobeIcon = () => (
  ...
  return (
    <Link href={`/${oppositeLocale}${pathname.substring(3) || ''}`} className="ml-4 rtl:ml-0 rtl:mr-4 flex items-center px-3 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 transition-colors shadow-sm">
