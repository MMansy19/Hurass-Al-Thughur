...
export function useBundleAnalyzer(budgets?: PerformanceBudget) {
  ...
  const analyzer = useMemo(
    () => BundleAnalyzer.getInstance(budgets),
    [budgets],
  );
...
export function BundleAnalysisDashboard({
  ...
  const { analysis, isLoading, error, runAnalysis, budgetStatus, formatBytes } =
    useBundleAnalyzer(budgets);
    ...
    <div className={`p-6 bg-white rounded-lg shadow-md ${className}`}>
      ...
      {budgetStatus && budgetStatus.exceeded && (
        <div className="bg-red-50 border border-red-200 p-4 rounded-lg mb-6">
          <h4 className="font-medium text-red-900 mb-2">Budget Violations</h4>{" "}
      ...
      {/* Largest Chunks */}
      <div className="mb-6">
        <h4 className="font-medium text-gray-900 mb-3">Largest Chunks</h4>{" "}
        <div className="space-y-2">
          {analysis.largestChunks
            .slice(0, 5)
            .map((chunk: { name: string; size: number }) => (
              <div
                key={chunk.name}
...
export function AdvancedBundleAnalysis({
  className = "",
  budgets,
}: AdvancedBundleAnalysisProps) {
