...
export const Stagger = memo<StaggerProps>(
  ({ children, staggerDelay = 0.1, className = "", preset = "slideInUp" }) => {
    return (
...
export const PageTransition = memo<PageTransitionProps>(
  ({ children, className = "" }) => {
    return (
      <Motion preset="fadeIn" duration={0.4} className={className}>
        {children}
      </Motion>
    );
  },
...
export const Parallax = memo<ParallaxProps>(
  ({ children, offset = 0.5, className = "" }) => {
    ...
    return (
      <div ref={elementRef} className={className} style={{ transform }}>
        {children}
      </div>
    );
  },
      ...
      >
        ...
        >
          <div className="flex items-center justify-center space-x-2">
            {loading && (
              <LoadingAnimation type="spinner" size="sm" color="white" />
            )}
