...
export const MagazineCard = memo<MagazineCardProps>(
  ({ issue, readNowText, onReadClick }) => {
    ...
    return (
      ...
      >
        <div
          className="aspect-[3/4] relative bg-gray-50"
          role="img"
          aria-label={`Cover image - ${issue.title}`}
        >
        ...
        <div className="p-5">
          <h3
            ...
            className="font-bold text-xl mb-2 text-emerald-900"
          >
            {issue.title}
          </h3>
          <p
            id={`issue-date-${issue.id}`}
            className="text-emerald-700 text-sm mb-3 flex items-center"
          >
            <svg
...
export const CategoryLink = memo<CategoryLinkProps>(
  ({ category, locale, browseCategoryText }) => {
    const ariaLabel = useMemo(
...
export const OptimizedPDFViewer = memo<OptimizedPDFViewerProps>(
  ({ pdfUrl, title, messages }) => {
    return (
      ...
      >
        <LazyPDFViewer pdfUrl={pdfUrl} title={title} messages={messages} />
      </Suspense>
    );
  },
);
