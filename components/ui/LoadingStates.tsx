...
export const LoadingButton = memo<LoadingButtonProps>(
  ({ loading, children, className = "", onClick, disabled = false }) => (
    <button
      onClick={onClick}
    ...
    >
      {loading && (
        <LoadingSpinner size="sm" color="white" className="absolute left-3" />
      )}
...
export const ProgressLoading = memo<ProgressLoadingProps>(
  ({ progress, showPercentage = true, className = "", size = "md" }) => {
    const sizeClasses = useMemo(() => {
      switch (size) {
        case "sm":
          return "h-2";
    ...
    return (
      <div className={`w-full ${className}`}>
        <div
          className={`bg-gray-200 rounded-full ${sizeClasses} overflow-hidden`}
        >
          <div
...
export const SkeletonTitle = memo<SkeletonProps>(({ className = "" }) => (
  <div
    className={`animate-pulse bg-gray-200  rounded h-6 w-3/4 ${className}`}
  />
));
...
export const SkeletonAvatar = memo<SkeletonProps>(({ className = "" }) => (
  <div
    className={`animate-pulse bg-gray-200  rounded-full w-10 h-10 ${className}`}
  />
));
...
export const SkeletonImage = memo<SkeletonProps>(({ className = "" }) => (
  <div
    className={`animate-pulse bg-gray-200  rounded-lg w-full h-48 ${className}`}
  />
));
...
export const MagazineGridSkeleton = memo<MagazineGridSkeletonProps>(
  ({ count = 6, columns = 3 }) => {
    ...
    return <div className={`grid ${gridClasses} gap-6`}>{items}</div>;
  },
);
...
export const CategoryFilterSkeleton = memo(() => (
  <div className="flex flex-wrap gap-2">
    {Array.from({ length: 6 }, (_, index) => (
      <SkeletonButton
        key={index}
        className={`h-8 ${index === 0 ? "w-20" : `w-${16 + (index % 3) * 4}`}`}
      />
    ))}
  </div>
));
        ...
        <>
          {emptyState || (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 ">لم يتم العثور على أي محتوى للعرض</p>
            </div>
          )}
        </>
      );
